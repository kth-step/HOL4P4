...
The current stack frame $\currsf$, is a stack of scopes where the global scope variables are stored at the bottom in location $\currsf$[0], whereas the variables of the current scope that being executed are stored on the top $\currsf$, and it is created whenever a new scope$\{ \}$ is entered. It represents a partial function that maps the variables names to their values.
\\~\\
The call stack $\cstack$, is a list of stack frames used wherever a function call occurs. Whenever the execution reaches a function call, the caller scope in location $\currsf$[i] will be stored in the $\cstack$, and later retrieved from $\cstack$ and store back in $\currsf$[i].  
\\~\\
The state memory $\stacks{}$ consists of a tuple of ($\currsf$, $\cstack$), to represent the live values of the program at a certain execution point of some instruction.
\\~\\
...
\\~\\
The execution state $\state{}$ holds a the state memory $\stacks{}$ and the status. Note that none of these constructs are laid out in the P4 specification, but rather made up by yours truly in order to obtain a formal P4 semantics.
...
The \textsc{stmt\_ass\_const} rule handles assignment statement. In general, the variables that the program can assign values to it should be in the global scope frame or the current scope frame, thus we need to look up into the current stack frame $\currsf$, but never into $\cstack$. So the antecedent $index = max  \{ j . x  \in \mathrm{dom} ( $\currsf$ [j] ) $ fetches the proper index that locates the variable location, it should be the one in the lowermost part of the the current stack frame $\currsf$ (i.e. oldest entry). In the last antecedent, it looks scary lol but this is the breakdown of it, first $ (x \longrightarrow V )scope $ updates the mapping of the variable name x to the new value i.e. (constant V) in the proper frame location, that indeed lies in the current stack frame. One step reduction in this rule results an updated current stack frame, with an empty statement to execute.
\\~\\
