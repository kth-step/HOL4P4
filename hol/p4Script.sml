...
(* TODO: Fix the silly recursive calls for struct and header now that termination proof isn't an issue *)
...
(init_out_v random_oracle i (v_bool boolv) = (v_bool $ random_oracle i, i+1)) /\
  ...
  (init_out_v random_oracle i (v_struct ((x,v)::t)) =
   let
  ...
  (init_out_v random_oracle i (v_struct []) = (v_struct [], i)) /\
  (init_out_v random_oracle i (v_header boolv ((x,v)::t)) =
   let
    (v', i') = init_out_v random_oracle i v
   in
   let
  ...
  (init_out_v random_oracle i (v_header boolv []) = (v_header F [], i)) /\
...
Definition init_from_tau_def:
  ...
  (init_from_tau random_oracle i (tau_xtl struct_ty_struct [] )             =  (v_struct [], i)) /\ 
  (init_from_tau random_oracle i (tau_xtl struct_ty_struct ((x0,t0)::xtl) ) =
   let
  ...
  (init_from_tau random_oracle i (tau_xtl struct_ty_header [] )            =  (v_header F [], i)) /\            
  (init_from_tau random_oracle i (tau_xtl struct_ty_header ((x0,t0)::xtl)) =
   let
    (v', i') = init_from_tau random_oracle i t0
   in
   let
...
val declare_list_in_scope_def = Define `
 declare_list_in_scope (t_scope:t_scope, scope:scope, i_opt, i, random_oracle) =
  let 
   (scope', i''') = FOLDR (\(x, (t,lvalop)) (f, i'). let (v', i'') = init_from_tau random_oracle i' t in (AUPDATE f (x , (v', NONE)), i'')) (scope:scope, case i_opt of NONE => i | SOME i_upd => i_upd) t_scope
  in
