...
Type get_oracle_index = ``:('a -> num)``

Type set_oracle_index = ``:(num option -> 'a -> 'a)``

Type input_f = ``:((in_out_list # 'a) -> (in_out_list # 'a) option)``

Type output_f = ``:((in_out_list # 'a) -> (in_out_list # 'a) option)``

Type copyin_pbl = ``:((x list # d list # e list # 'a # random_oracle) -> (scope # num option) option)``
...
Type e_red_res = ``:(frame_list # (num option))``
...
Definition get_oracle_calls_def:
 (get_oracle_calls 0 i f = []) /\
 (get_oracle_calls (SUC n) i f =
  (f i)::(get_oracle_calls n (i+1) f)
 )
End

Definition init_out_v_def:
 (init_out_v random_oracle i (v_bool boolv) = (v_bool $ random_oracle i, i+1)) /\
 (init_out_v random_oracle i (v_bit (bl, n)) = (v_bit (get_oracle_calls n i random_oracle, n), i+n)) /\
 (* Note you can't have string variables in P4 (but you can pass string literals to externs) *)
 (init_out_v random_oracle i (v_str x) = (v_str "", i)) /\
 (init_out_v random_oracle i (v_struct x_v_l) =
  let
   (x_v_l'', i''') = FOLDL ( \ (x_v_l', i') (x, v). let (v', i'') = init_out_v random_oracle i' v in ((x, v')::x_v_l', i'')) ([], i) x_v_l
  in
   (v_struct $ REVERSE x_v_l'', i''')
 ) /\
 (init_out_v random_oracle i (v_header boolv x_v_l) =
  let
   (x_v_l'', i''') = FOLDL ( \ (x_v_l', i') (x, v). let (v', i'') = init_out_v random_oracle i' v in ((x, v')::x_v_l', i'')) ([], i) x_v_l
  in
   (v_header F (REVERSE x_v_l''), i''')
 ) /\
 (init_out_v random_oracle i (v_ext_ref i') = (v_ext_ref i', i)) /\
 (init_out_v random_oracle i v_bot = (v_bot, i))
Termination
WF_REL_TAC ‘measure ( \ (a,b,c). v_size c)’
End
...
Theorem tau1_size_eq:
!tau_l. tau1_size tau_l = list_size (pair_size (list_size char_size) tau_size) tau_l
Proof
Induct >- (
 gs[tau_size_def]
) >>
...
(* Initialise values for a given type *)
Definition init_from_tau_def:
 (init_from_tau random_oracle i tau_bool     = (v_bool $ random_oracle i, i+1)) /\
 (init_from_tau random_oracle i (tau_bit w)  = (v_bit (get_oracle_calls w i random_oracle, w), i+w)) /\
 (init_from_tau random_oracle i tau_bot      = (v_bot, i)) /\
 (* NOTE: Extern objects can't be declared in blocks.
  *       They can be block-local, but will then always be instantiated, so this value doesn't matter *)
 (init_from_tau random_oracle i tau_ext      = (v_ext_ref 0, i)) /\
 (init_from_tau random_oracle i (tau_xtl struct_ty_struct xtl) =
  let
   (x_v_l', i''') = FOLDL ( \ (x_v_l, i') (x, tau). let (v, i'') = init_from_tau random_oracle i' tau in ((x, v)::x_v_l, i'')) ([], i) xtl
  in
   (v_struct $ REVERSE x_v_l', i''')
 ) /\
 (init_from_tau random_oracle i (tau_xtl struct_ty_header xtl) =
  let
   (x_v_l', i''') = FOLDL ( \ (x_v_l, i') (x, tau). let (v, i'') = init_from_tau random_oracle i' tau in ((x, v)::x_v_l, i'')) ([], i) xtl
  in
   (v_header F (REVERSE x_v_l'), i''')
...
Termination
WF_REL_TAC ‘measure ( \ (a,b,c). tau_size c)’ >>
rpt strip_tac >> (
...
val one_arg_val_for_newscope_def = Define `
 one_arg_val_for_newscope d e ss i random_oracle =
  if is_d_out d
  then
   (case get_lval_of_e e of
    | SOME lval =>
     (case lookup_lval ss lval of
      | SOME v =>
       if is_d_in d
       then SOME ((v, i), SOME lval)
       else SOME (init_out_v random_oracle i v, SOME lval)
      | NONE => NONE)
    | NONE => NONE)
  else
   (case v_of_e e of
    | SOME v => SOME ((v, i), NONE)
    | NONE => NONE)
`;
...
val update_arg_for_newscope_def = Define `
  update_arg_for_newscope ss random_oracle f_i_opt (d, x, e) =
    case f_i_opt of
    | SOME (f,i) =>
      (case one_arg_val_for_newscope d e ss i random_oracle of
       | SOME ((v, i'), lval_opt) => SOME (AUPDATE f (varn_name x, (v, lval_opt)), i')
       | NONE => NONE)
    | NONE => NONE
`;
...
val all_arg_update_for_newscope_def = Define `
  all_arg_update_for_newscope xlist dlist elist ss i random_oracle = 
    FOLDL (update_arg_for_newscope ss random_oracle) (SOME ([], i)) (ZIP (dlist, ZIP(xlist, elist)))
`;


(* full copyin definition *)
val copyin_def = Define `
  copyin xlist dlist elist gsl ss_curr i random_oracle =
   case all_arg_update_for_newscope xlist dlist elist (ss_curr++gsl) i random_oracle of
   | SOME (scope', i') =>
    SOME (scope', if i = i' then NONE else SOME i')
   | NONE => NONE
`;
...
Definition get_ectx_def:
 get_ectx ((apply_table_f, ext_map, func_map, b_func_map, pars_map, tbl_map, get_oracle_index, set_oracle_index, random_oracle):'a ctx) (ascope:'a) = (apply_table_f, ext_map, func_map, b_func_map, pars_map, tbl_map, get_oracle_index ascope, random_oracle)
End
...
val declare_list_in_scope_def = Define `
 declare_list_in_scope (t_scope:t_scope, scope:scope, i_opt, i, random_oracle) =
  let 
   (scope', i''') = FOLDR (\(x, (t,lvalop)) (f, i'). let (v', i'') = init_from_tau random_oracle i' t in ((x , (v', NONE))::f, i'')) (scope:scope, case i_opt of NONE => i | SOME i_upd => i_upd) t_scope
  in
   (scope', if i = i''' then NONE else SOME i''')
`;
...
val declare_list_in_fresh_scope_def = Define `
 declare_list_in_fresh_scope (t_scope:t_scope, i, random_oracle) =
  declare_list_in_scope (t_scope:t_scope, [], NONE, i, random_oracle)
`;
...
[e_lookup:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (varn:varn) (v:v) .
(clause_name "e_lookup") /\
(( SOME  v  =  lookup_vexp2  scope_list   g_scope_list   varn ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_var varn) (e_v v)  ([], (NONE:num option))  )))

[e_call_newframe:] (! (e_x_d_list:(e#x#d) list) (apply_table_f:'a apply_table_f) (ext_map:'a ext_map) (func_map:func_map) (b_func_map:b_func_map) (pars_map:pars_map) (tbl_map:tbl_map) (i:i) (random_oracle:random_oracle) (g_scope_list:g_scope_list) (scope_list:scope_list) (funn:funn) (stmt:stmt) (scope':scope) (i_opt:i_opt) .
...
( (SOME ( scope' ,  i_opt ) = copyin  ((MAP (\(e_,x_,d_) . x_) e_x_d_list))    ( ((MAP (\(e_,x_,d_) . d_) e_x_d_list)) )     ( ((MAP (\(e_,x_,d_) . e_) e_x_d_list)) )    g_scope_list   scope_list   i   random_oracle ) ))
 ==> 
...
( ( e_red ectx g_scope_list scope_list e e' e_red_res )) /\
( (  ( ((MAP (\(f_,e_,e'_) . e'_) f_e_e'_list)) )   =   (LUPDATE  e'   i    ( ((MAP (\(f_,e_,e'_) . e_) f_e_e'_list)) )  )  ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_struct ((MAP (\(f_,e_,e'_) . (f_,e_)) f_e_e'_list))) (e_struct ((MAP (\(f_,e_,e'_) . (f_,e'_)) f_e_e'_list))) e_red_res )))

[e_eStruct_to_v:] (! (f_e_v_list:(x#e#v) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) .
...
( (  ( ((MAP (\(f_,e_,v_) . v_) f_e_v_list)) )   = vl_of_el   ( ((MAP (\(f_,e_,v_) . e_) f_e_v_list)) )  ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_struct ((MAP (\(f_,e_,v_) . (f_,e_)) f_e_v_list))) (e_v (v_struct ((MAP (\(f_,e_,v_) . (f_,v_)) f_e_v_list))))  ([], (NONE:num option))  )))

[e_s_acc:] (! (f_v_list:(x#v) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (f:x) (v:v) .
(clause_name "e_s_acc") /\
(( (FIND (\(k, v). k =  f )  (f_v_list)  = SOME ( f ,  v )) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_acc (e_v (v_struct (f_v_list))) f) (e_v v)  ([], (NONE:num option))  )))

[e_acc_arg1:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) (f:x) (e':e) (e_red_res:e_red_res) .
(clause_name "e_acc_arg1") /\
(( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_acc e f) (e_acc e' f) e_red_res )))

[e_eHeader:] (! (f_e_e'_list:(x#e#e) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (boolv:boolv) (e_red_res:e_red_res) (i:i) (e:e) (e':e) .
...
( (  ( ((MAP (\(f_,e_,e'_) . e'_) f_e_e'_list)) )   =   (LUPDATE  e'   i    ( ((MAP (\(f_,e_,e'_) . e_) f_e_e'_list)) )  )  ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_header boolv ((MAP (\(f_,e_,e'_) . (f_,e_)) f_e_e'_list))) (e_header boolv ((MAP (\(f_,e_,e'_) . (f_,e'_)) f_e_e'_list))) e_red_res )))

[e_eHeader_to_v:] (! (f_e_v_list:(x#e#v) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (boolv:boolv) .
(clause_name "e_eHeader_to_v") /\
(( (is_consts   ( ((MAP (\(f_,e_,v_) . e_) f_e_v_list)) )  ) ) /\
( (  ( ((MAP (\(f_,e_,v_) . v_) f_e_v_list)) )   = vl_of_el   ( ((MAP (\(f_,e_,v_) . e_) f_e_v_list)) )  ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_header boolv ((MAP (\(f_,e_,v_) . (f_,e_)) f_e_v_list))) (e_v (v_header boolv ((MAP (\(f_,e_,v_) . (f_,v_)) f_e_v_list))))  ([], (NONE:num option))  )))

[e_h_acc:] (! (f_v_list:(x#v) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (boolv:boolv) (f:x) (v:v) .
(clause_name "e_h_acc") /\
(( (FIND (\(k, v). k =  f )  (f_v_list)  = SOME ( f ,  v )) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_acc (e_v (v_header boolv (f_v_list))) f) (e_v v)  ([], (NONE:num option))  )))

[e_sel_acc:] (! (s_list_x_list:(s_list#x) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (v:v) (x:x) (x':x) .
(clause_name "e_sel_acc") /\
(( x'  = sel  v   (s_list_x_list)   x ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_select (e_v v) (s_list_x_list) x) (e_v (v_str x'))  ([], (NONE:num option))  )))

[e_concat_arg1:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) (e':e) (e'':e) (e_red_res:e_red_res) .
(clause_name "e_concat_arg1") /\
(( ( e_red ectx g_scope_list scope_list e e'' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_concat e e') (e_concat e'' e') e_red_res )))

[e_concat_arg2:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (e:e) (e':e) (e_red_res:e_red_res) .
(clause_name "e_concat_arg2") /\
(( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_concat (e_v (v_bit bitv)) e) (e_concat (e_v (v_bit bitv)) e') e_red_res )))

[e_concat_v:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_concat_v") /\
(( ( bitv''  = bitv_concat  bitv   bitv' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_concat (e_v (v_bit bitv)) (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_slice_arg1:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) (bitv:bitv) (bitv':bitv) (e':e) (e_red_res:e_red_res) .
(clause_name "e_slice_arg1") /\
(( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_slice e (e_v (v_bit bitv)) (e_v (v_bit bitv'))) (e_slice e' (e_v (v_bit bitv)) (e_v (v_bit bitv'))) e_red_res )))

[e_slice_v:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) (bitv''':bitv) .
(clause_name "e_slice_v") /\
(( bitv'''  = slice  bitv   bitv'   bitv'' ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_slice (e_v (v_bit bitv)) (e_v (v_bit bitv')) (e_v (v_bit bitv''))) (e_v (v_bit bitv'''))  ([], (NONE:num option))  )))

[e_sel_arg:] (! (s_list_x_list:(s_list#x) list) (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) (x:x) (e':e) (e_red_res:e_red_res) .
(clause_name "e_sel_arg") /\
(( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_select e (s_list_x_list) x) (e_select e' (s_list_x_list) x) e_red_res )))

[e_unop_arg:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (unop:unop) (e:e) (e':e) (e_red_res:e_red_res) .
(clause_name "e_unop_arg") /\
(( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_unop unop e) (e_unop unop e') e_red_res )))

[e_cast_arg:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (cast:cast) (e:e) (e':e) (e_red_res:e_red_res) .
(clause_name "e_cast_arg") /\
(( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_cast cast e) (e_cast cast e') e_red_res )))

[e_binop_arg1:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) (binop:binop) (e':e) (e'':e) (e_red_res:e_red_res) .
(clause_name "e_binop_arg1") /\
(( ( e_red ectx g_scope_list scope_list e e'' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop e binop e') (e_binop e'' binop e') e_red_res )))

[e_binop_arg2:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (v:v) (binop:binop) (e:e) (e':e) (e_red_res:e_red_res) .
(clause_name "e_binop_arg2") /\
(( (~is_short_circuitable  binop ) ) /\
( ( e_red ectx g_scope_list scope_list e e' e_red_res )))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v v) binop e) (e_binop (e_v v) binop e') e_red_res )))

[e_neg_bool:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (b:b) (b':b) .
(clause_name "e_neg_bool") /\
(( (~ b  =  b' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_unop unop_neg (e_v (v_bool  b ))) (e_v (v_bool  b' ))  ([], (NONE:num option))  )))

[e_compl:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) .
(clause_name "e_compl") /\
(( (bitv_bl_unop bnot  bitv  =  bitv' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_unop unop_compl (e_v (v_bit bitv))) (e_v (v_bit bitv'))  ([], (NONE:num option))  )))

[e_neg_signed:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) .
(clause_name "e_neg_signed") /\
(( (bitv_unop unop_neg_signed  bitv  =  bitv' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_unop unop_neg_signed (e_v (v_bit bitv))) (e_v (v_bit bitv'))  ([], (NONE:num option))  )))

[e_un_plus:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) .
(clause_name "e_un_plus") /\
(( ( bitv  =  bitv' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_unop unop_un_plus (e_v (v_bit bitv))) (e_v (v_bit bitv'))  ([], (NONE:num option))  )))

[e_cast_bitv:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (n:m) (bitv:bitv) (bitv':bitv) .
(clause_name "e_cast_bitv") /\
(( (bitv_cast  n   bitv  =  bitv' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_cast (cast_unsigned n) (e_v (v_bit bitv))) (e_v (v_bit bitv'))  ([], (NONE:num option))  )))

[e_cast_bool:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (n:m) (b:b) (bitv:bitv) .
(clause_name "e_cast_bool") /\
(( (bool_cast  n   b  =  bitv ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_cast (cast_unsigned n) (e_v (v_bool  b ))) (e_v (v_bit bitv))  ([], (NONE:num option))  )))

[e_cast_to_bool:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (b:b) .
(clause_name "e_cast_to_bool") /\
(( (to_bool_cast  bitv  =  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_cast cast_bool (e_v (v_bit bitv))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_cast_id_bool:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (b:b) (b':b) .
(clause_name "e_cast_id_bool") /\
(( (id_bool_cast  b  =  b' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_cast cast_bool (e_v (v_bool  b ))) (e_v (v_bool  b' ))  ([], (NONE:num option))  )))

[e_mul:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_mul") /\
(( (bitv_binop binop_mul  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_mul (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_div:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_div") /\
(( (bitv_binop binop_div  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_div (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_mod:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_mod") /\
(( (bitv_binop binop_mod  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_mod (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_add:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_add") /\
(( (bitv_binop binop_add  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_add (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_sat_add:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_sat_add") /\
(( (bitv_binop binop_sat_add  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_sat_add (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_sub:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_sub") /\
(( (bitv_binop binop_sub  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_sub (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_sat_sub:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_sat_sub") /\
(( (bitv_binop binop_sat_sub  bitv   bitv'  = SOME  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_sat_sub (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_shl:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_shl") /\
(( (bitv_bl_binop shiftl  bitv  ((\(bl, n). (v2n bl, n))  bitv' ) =  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_shl (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_shr:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_shr") /\
(( (bitv_bl_binop shiftr  bitv  ((\(bl, n). (v2n bl, n))  bitv' ) =  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_shr (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_le:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (b:b) .
(clause_name "e_le") /\
(( ((bitv_binpred binop_le  bitv   bitv' ) = SOME  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_le (e_v (v_bit bitv'))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_ge:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (b:b) .
(clause_name "e_ge") /\
(( ((bitv_binpred binop_ge  bitv   bitv' ) = SOME  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_ge (e_v (v_bit bitv'))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_lt:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (b:b) .
(clause_name "e_lt") /\
(( ((bitv_binpred binop_lt  bitv   bitv' ) = SOME  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_lt (e_v (v_bit bitv'))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_gt:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (b:b) .
(clause_name "e_gt") /\
(( ((bitv_binpred binop_gt  bitv   bitv' ) = SOME  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_gt (e_v (v_bit bitv'))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_neq:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (b:b) .
(clause_name "e_neq") /\
(( (( bitv  <>  bitv' ) <=>  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_neq (e_v (v_bit bitv'))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_neq_bool:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (b:b) (b':b) (b'':b) .
(clause_name "e_neq_bool") /\
(( (( b  <>  b' ) <=>  b'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bool  b )) binop_neq (e_v (v_bool  b' ))) (e_v (v_bool  b'' ))  ([], (NONE:num option))  )))

[e_eq:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (b:b) .
(clause_name "e_eq") /\
(( (( bitv  =  bitv' ) <=>  b ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_eq (e_v (v_bit bitv'))) (e_v (v_bool  b ))  ([], (NONE:num option))  )))

[e_eq_bool:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (b:b) (b':b) (b'':b) .
(clause_name "e_eq_bool") /\
(( ( b  =  b'  <=>  b'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bool  b )) binop_eq (e_v (v_bool  b' ))) (e_v (v_bool  b'' ))  ([], (NONE:num option))  )))

[e_and:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_and") /\
(( (bitv_bl_binop band  bitv   bitv'  =  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_and (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_xor:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_xor") /\
(( (bitv_bl_binop bxor  bitv   bitv'  =  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_xor (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_or:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (bitv:bitv) (bitv':bitv) (bitv'':bitv) .
(clause_name "e_or") /\
(( (bitv_bl_binop bor  bitv   bitv'  =  bitv'' ) ))
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bit bitv)) binop_or (e_v (v_bit bitv'))) (e_v (v_bit bitv''))  ([], (NONE:num option))  )))

[e_bin_and1:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) .
(clause_name "e_bin_and1")
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bool  F )) binop_bin_and e) (e_v (v_bool  F ))  ([], (NONE:num option))  )))

[e_bin_and2:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) .
(clause_name "e_bin_and2")
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bool  T )) binop_bin_and e) e  ([], (NONE:num option))  )))

[e_bin_or1:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) .
(clause_name "e_bin_or1")
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bool  T )) binop_bin_or e) (e_v (v_bool  T ))  ([], (NONE:num option))  )))

[e_bin_or2:] (! (ectx:'a ectx) (g_scope_list:g_scope_list) (scope_list:scope_list) (e:e) .
(clause_name "e_bin_or2")
 ==> 
( ( e_red ectx g_scope_list scope_list (e_binop (e_v (v_bool  F )) binop_bin_or e) e  ([], (NONE:num option))  )))
End
(** definitions *)
...
[stmt_block_enter:] (! (apply_table_f:'a apply_table_f) (ext_map:'a ext_map) (func_map:func_map) (b_func_map:b_func_map) (pars_map:pars_map) (tbl_map:tbl_map) (get_oracle_index:'a get_oracle_index) (set_oracle_index:'a set_oracle_index) (random_oracle:random_oracle) (ascope:'a) (g_scope_list:g_scope_list) (funn:funn) (t_scope:t_scope) (stmt:stmt) (scope_list:scope_list) (i_opt:i_opt) (scope_list':scope_list) (scope:scope) .
(clause_name "stmt_block_enter") /\
(( ( scope ,  i_opt ) = declare_list_in_fresh_scope ( t_scope ,  get_oracle_index   ascope  ,  random_oracle ) ) /\
( ( scope_list'  =   (  ( ([(scope)]) )   ++  scope_list )  ) ))
 ==> 
( ( stmt_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,   ([   ( funn  ,   ( ([((stmt_block t_scope stmt))]) )   ,  scope_list )   ])  ,  status_running )   (set_oracle_index  i_opt   ascope ,  g_scope_list ,   ([   ( funn  ,   ( stmt  ::   ( ([(stmt_empty)]) )  )   ,  scope_list' )   ])  ,  status_running )  )))
...
(( ( stmt  <> stmt_empty) ) /\
( ( stmt_stack  <> []) ) /\
( ( stmt_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,   ([   ( funn  ,   ( ([(stmt)]) )   ,  scope_list )   ])  ,  status )   ( ascope' ,  g_scope_list' ,   ( frame_list'  ++   ([   ( funn  ,  stmt_stack'  ,  scope_list' )   ])  )  ,  status' )  )))
 ==> 
( ( stmt_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,   ([   ( funn  ,   ( stmt  ::  stmt_stack )   ,  scope_list )   ])  ,  status )   ( ascope' ,  g_scope_list' ,   ( frame_list'  ++   ([   ( funn  ,   ( stmt_stack'  ++  stmt_stack )   ,  scope_list' )   ])  )  ,  status' )  )))
...
[stmt_apply_table_v:] (! (e'_list:e list) (e_mk_list:(e#mk) list) (v_list:v list) (apply_table_f:'a apply_table_f) (ext_map:'a ext_map) (func_map:func_map) (b_func_map:b_func_map) (pars_map:pars_map) (tbl_map:tbl_map) (get_oracle_index:'a get_oracle_index) (set_oracle_index:'a set_oracle_index) (random_oracle:random_oracle) (ascope:'a) (g_scope_list:g_scope_list) (funn:funn) (tbl:x) (scope_list:scope_list) (f:x) (f':x) .
...
( ( apply_table_f  (  tbl  ,   (  ( ((MAP (\(e_,mk_) . e_) e_mk_list)) )  )   ,   (  ( ((MAP (\(e_,mk_) . mk_) e_mk_list)) )  )   , (  f'  ,   ( (e'_list) )   ),  ascope ) = SOME (  f  ,   (  ( ((MAP (\v_ . (e_v v_)) v_list)) )  )   ) ) ))
 ==> 
( ( stmt_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,   ([   ( funn  ,   ( ([((stmt_app tbl ((MAP (\(e_,mk_) . e_) e_mk_list))))]) )   ,  scope_list )   ])  ,  status_running )   ( ascope ,  g_scope_list ,   ([   ( funn  ,   ( ([((stmt_ass lval_null  ( (e_call (funn_name f) ((MAP (\v_ . (e_v v_)) v_list))) ) ))]) )   ,  scope_list )   ])  ,  status_running )  )))
...
( ( stmt_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map'  ,  pars_map ,  tbl_map' ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list' ,   ([   ( funn  ,  stmt_stack  ,  scope_list )   ])  ,  status )   ( ascope' ,  g_scope_list'' ,  frame_list' ,  status' )  )) /\
( ( ( frame_list''  <> []) ==> notret  status'  ) ) /\
( (SOME g_scope_list'''  = scopes_to_retrieve  funn   func_map   b_func_map   g_scope_list   g_scope_list'' ) ))
 ==> 
( ( frames_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,   (  ([   ( funn  ,  stmt_stack  ,  scope_list )   ])   ++  frame_list'' )  ,  status )   ( ascope' ,  g_scope_list''' ,   ( frame_list'  ++  frame_list'' )  ,  status' )  )))
...
( SOME  b_func_map'  = map_to_pass  funn   b_func_map ) /\
( SOME  tbl_map'  = tbl_to_pass  funn   b_func_map   tbl_map ) /\
( ( stmt_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map'  ,  pars_map ,  tbl_map' ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list' ,   ([   ( funn  ,  stmt_stack  ,  scope_list )   ])  ,  status_running )   ( ascope' ,  g_scope_list'' ,   ([   ( funn  ,  stmt_stack''  ,  scope_list'' )   ])  ,  (status_returnv v) )  )) /\
...
( (SOME g_scope_list'''''''  = scopes_to_retrieve  funn'   func_map   b_func_map   g_scope_list''''   g_scope_list'''''' ) ))
 ==> 
( ( frames_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,   (  ([   ( funn  ,  stmt_stack  ,  scope_list )   ])   ++   (  (  ([   ( funn'  ,  stmt_stack'  ,  scope_list' )   ])   ++  frame_list )  )  )  ,  status_running )   ( ascope' ,  g_scope_list''''''' ,   (  ([   ( funn'  ,  stmt_stack'  ,  scope_list''' )   ])   ++  frame_list )  ,  status_running )  )))
End
(** definitions *)
...
[arch_pbl_init:] (! (e_x_d_list:(e#x#d) list) (ab_list:ab_list) (pblock_map:pblock_map) (ffblock_map:'a ffblock_map) (input_f:'a input_f) (output_f:'a output_f) (copyin_pbl:'a copyin_pbl) (copyout_pbl:'a copyout_pbl) (apply_table_f:'a apply_table_f) (ext_map:'a ext_map) (func_map:func_map) (get_oracle_index:'a get_oracle_index) (set_oracle_index:'a set_oracle_index) (random_oracle:random_oracle) (i:i) (in_out_list:in_out_list) (in_out_list':in_out_list) (ascope:'a) (g_scope_list:g_scope_list) (i_opt':i_opt) (g_scope_list''':g_scope_list) (f:x) (stmt:stmt) (pbl_type:pbl_type) (b_func_map:b_func_map) (t_scope:t_scope) (pars_map:pars_map) (tbl_map:tbl_map) (scope':scope) (i_opt:i_opt) (scope'':scope) (g_scope_list':g_scope_list) (g_scope_list'':g_scope_list) .
...
( (SOME ( scope' ,  i_opt ) =  copyin_pbl  ( ((MAP (\(e_,x_,d_) . x_) e_x_d_list)) ,   ( ((MAP (\(e_,x_,d_) . d_) e_x_d_list)) )  ,   ( ((MAP (\(e_,x_,d_) . e_) e_x_d_list)) )  ,  ascope ,  random_oracle )) ) /\
( ( scope'' ,  i_opt' ) = declare_list_in_scope ( t_scope ,  scope' ,  i_opt  ,  get_oracle_index   ascope  ,  random_oracle ) ) /\
...
( (ALOOKUP  pars_map   x'  = SOME ( stmt' )) ))
 ==> 
( ( arch_red  ( ab_list ,  pblock_map ,  ffblock_map ,  input_f ,  output_f ,  copyin_pbl ,  copyout_pbl ,  apply_table_f ,  ext_map ,  func_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   (  (  i  ,  in_out_list ,  in_out_list' ,  ascope )  ,  g_scope_list ,  (arch_frame_list_regular frame_list) ,  (status_trans x') )   (  (  i  ,  in_out_list ,  in_out_list' ,  ascope )  ,  g_scope_list' ,  (arch_frame_list_regular  ([   ( (funn_name x')  ,   ( ([(stmt')]) )   ,   ( ([( [] )]) )  )   ]) ) ,  status_running )  )))
...
( ( frames_red  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ( ascope ,  g_scope_list ,  frame_list ,  status_running )   ( ascope' ,  g_scope_list' ,  frame_list' ,  status' )  )))
 ==> 
( ( arch_red  ( ab_list ,  pblock_map ,  ffblock_map ,  input_f ,  output_f ,  copyin_pbl ,  copyout_pbl ,  apply_table_f ,  ext_map ,  func_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   (  (  i  ,  in_out_list ,  in_out_list' ,  ascope )  ,  g_scope_list ,  (arch_frame_list_regular frame_list) ,  status_running )   (  (  i  ,  in_out_list ,  in_out_list' ,  ascope' )  ,  g_scope_list' ,  (arch_frame_list_regular frame_list') ,  status' )  )))
...
( (SOME  ascope'  =  copyout_pbl  (  (  g_scope_list  )  ,  ascope ,   ( ((MAP (\(e_,x_,d_) . d_) e_x_d_list)) )  ,  ((MAP (\(e_,x_,d_) . x_) e_x_d_list)) ,  status' )) ))
 ==> 
( ( arch_red  ( ab_list ,  pblock_map ,  ffblock_map ,  input_f ,  output_f ,  copyin_pbl ,  copyout_pbl ,  apply_table_f ,  ext_map ,  func_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   (  (  i  ,  in_out_list ,  in_out_list' ,  ascope )  ,  g_scope_list ,  (arch_frame_list_regular frame_list) ,  status )   (  (  (   i   +   1   )  ,  in_out_list ,  in_out_list' ,  ascope' )  ,   (LASTN   1    g_scope_list )  ,  arch_frame_list_empty ,  status_running )  )))
End
(** definitions *)
...
[conc_conc1:] (! (ab_list:ab_list) (pblock_map:pblock_map) (ffblock_map:'a ffblock_map) (input_f:'a input_f) (output_f:'a output_f) (copyin_pbl:'a copyin_pbl) (copyout_pbl:'a copyout_pbl) (apply_table_f:'a apply_table_f) (ext_map:'a ext_map) (func_map:func_map) (get_oracle_index:'a get_oracle_index) (set_oracle_index:'a set_oracle_index) (random_oracle:random_oracle) (in_out_list:in_out_list) (in_out_list':in_out_list) (ascope:'a) (i:i) (g_scope_list:g_scope_list) (arch_frame_list:arch_frame_list) (status:status) (i':i) (g_scope_list':g_scope_list) (arch_frame_list':arch_frame_list) (status':status) (in_out_list'':in_out_list) (in_out_list''':in_out_list) (ascope':'a) (i'':i) (g_scope_list'':g_scope_list) (arch_frame_list'':arch_frame_list) (status'':status) .
(clause_name "conc_conc1") /\
(( ( arch_red  ( ab_list ,  pblock_map ,  ffblock_map ,  input_f ,  output_f ,  copyin_pbl ,  copyout_pbl ,  apply_table_f ,  ext_map ,  func_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   (  (  i  ,  in_out_list ,  in_out_list' ,  ascope )  ,  g_scope_list ,  arch_frame_list ,  status )   (  (  i''  ,  in_out_list'' ,  in_out_list''' ,  ascope' )  ,  g_scope_list'' ,  arch_frame_list'' ,  status'' )  )))
 ==> 
( ( conc_red  ( ab_list ,  pblock_map ,  ffblock_map ,  input_f ,  output_f ,  copyin_pbl ,  copyout_pbl ,  apply_table_f ,  ext_map ,  func_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )   ((   in_out_list  ,  in_out_list'  ,  ascope  ) , ( (   i   ,  g_scope_list  ,  arch_frame_list  ,  status  ) , (   i'   ,  g_scope_list'  ,  arch_frame_list'  ,  status'  ) ))   ((   in_out_list''  ,  in_out_list'''  ,  ascope'  ) , ( (   i''   ,  g_scope_list''  ,  arch_frame_list''  ,  status''  ) , (   i'   ,  g_scope_list'  ,  arch_frame_list'  ,  status'  ) ))  )))
...
( ( f_in_apply_tbl  tbl_map   apply_table_f  ) ))
 ==> 
( ( WT_c  ( apply_table_f ,  ext_map ,  func_map ,  b_func_map  ,  pars_map ,  tbl_map ,  get_oracle_index  ,  set_oracle_index  ,  random_oracle )  order t_scope_list_g delta_g delta_b delta_x delta_t Prs_n )))
End
