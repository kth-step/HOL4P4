...
Cases_on `arch_frame_list` >> (
 ...
 rpt strip_tac >>
 PairCases_on `actx` >>
 rename1 `(ab_list, pblock_map, ffblock_map, input_f, output_f, copyin_pbl, copyout_pbl, apply_table_f, ext_map, func_map, get_oracle_index, set_oracle_index, random_oracle)` >>
 PairCases_on `aenv` >>
 ...
 Cases_on `EL i ab_list` >| [
  ...
  fs[] >>
  Cases_on `copyin_pbl (MAP FST x_d_l,MAP SND x_d_l,l,ascope,random_oracle)` >> (
   fs[]
  ) >>
  PairCases_on ‘x’ >>
  gs[] >>
  ...
  Cases_on `declare_list_in_scope
             (t_scope,x0,x1,get_oracle_index ascope,random_oracle)` >> (
   fs[]
  ) >>
  Cases_on `initialise_var_stars func_map b_func_map ext_map [q; h]` >> (
   fs[]
  ) >>
  rw [] >>
  irule ((valOf o find_clause_arch_red) "arch_pbl_init") >>
  fs[clause_name_def] >>
  qexistsl_tac [`ZIP (l, ZIP(MAP FST x_d_l, MAP SND x_d_l))`, ‘x1’, `x0`, ‘q’] >>
  fs[] >>
  rpt strip_tac >| [
 ...
 Cases_on `state_fin status_running l` >| [
  ...
  Cases_on `frames_exec (apply_table_f,ext_map,func_map,x2,x4,x5,get_oracle_index,
