...
#This version is hard-coded to export to hol4p4exe format.

JSON_PATH=$1
LOG_PATH=$2

if [ "$#" -ne 2 ]; then
    echo "petr4_to_hol4p4exe.sh requires two arguments: the first a path to a P4 program in petr4 JSON format, the second a path to a log file"
    exit 1
fi
