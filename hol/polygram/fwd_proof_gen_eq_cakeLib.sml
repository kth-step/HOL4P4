...
(* ---------------------------------------------------------------------------
   gen_eq_policy_and_prove

   Implements certified policy minimization (Section VII), reusing the same
   MTBDD construction as the compilation pipeline (path purple).

   The input policy is translated to its policy ILR (Theorem 1 / Thm IF1),
   and its MTBDD is built via the verified CakeML binary (Theorem 3).
   The untrusted SML function mtbdd_to_rules then extracts a minimized
   policy ILR (var_policy2) directly from the MTBDD. A second CakeML run
   builds MTBDD2 from var_policy2, and isomorphism between the two MTBDDs
   certifies equivalence (Theorem 4). Finally, var_policy2 is translated
   back to an arithmetic policy (trans-back, Theorem 1 retranslation) and
   the end-to-end equivalence proof is composed.
   I/O serialization to/from the CakeML binary is TCB; see mk_oracle_thm.

   Arguments:
     arith_policy1     - input high-level forwarding policy with arithmetic
   ...
   Returns:
     ...
     where policy2 is the generated minimized policy.
   ...
   --------------------------------------------------------------------------- *)
    ...
    fun gen_eq_policy_and_prove (arith_policy1, policy_me, test_pd_type, policy_order, file_name) =
        ...
        (*                                                  *)
        (* trans-fwd replaces each atomic predicate in      *)
        (* arith_policy1 with its variable in m (policy_me).*)
        (* Theorem 1 proves soundness of trans-fwd.         *)
        (****************************************************)

        (* Apply trans-fwd to policy1. *)
        ...
        (* 2b. MTBDD1       -> policy ILR2  (untrusted SML  *)
        (*     mtbdd_to_rules: minimized policy extraction) *)
        (* 2c. policy ILR2  -> MTBDD2  (CakeML binary)     *)
        ...
        (* 2a. Serialize var_policy1 and policy_order to text files and run
               the verified CakeML binary to build MTBDD1 (Theorem 3).
               Serialization is part of the TCB (Section VI). *)
        ...
        (* Deserialize MTBDD1 from the CakeML output file back into a HOL4
        ...
        (* 2b. Extract a minimized policy ILR (var_policy2) from MTBDD1 using
               the untrusted SML function mtbdd_to_rules (Section VII).
               This step is untrusted; its correctness is validated a
               posteriori by the isomorphism check in step 2d. *)
        val var_policy2 = mtbdd_to_rules policy_bdd_content_term1;


        (* 2c. Serialize var_policy2 and run the verified CakeML binary to
               build MTBDD2 (Theorem 3). Serialization is part of the TCB. *)
        ...
        (* Deserialize MTBDD2 from the CakeML output file. TCB. *)
        ...
        (* 2d. Compute the candidate isomorphism witness I between MTBDD1
               and MTBDD2. *)
        ...
        (*           var policy ILR2 -> arith policy2       *)
        (*                                                  *)
        (* convert_var_to_arith_policy implements trans-back*)
        (* for the policy language: variables are replaced  *)
        (* by their atomic predicates in m (policy_me).     *)
        (* Theorem 1 retranslation proves soundness:        *)
        (*   for every u ~_m packet,                        *)
        (*     sem-down(var_policy2) u                      *)
        (*       = sem-up(arith_policy2) packet             *)
        (****************************************************)

        (* Apply trans-back to var_policy2 to obtain the minimized
           arithmetic policy arith_policy2. *)
        ...
        (* Instantiate Theorem 1 retranslation for policy2:
             |- for every u ~_m packet,
                  sem-down(var_policy2) u = sem-up(arith_policy2) packet  *)
        ...
        (*                                                  *)
        (* Composes Thm 1 (IF1), Thms 3+4, Thm 1 (trans-back):*)
        (*   ORACLE|- !packet. wf_packet T packet =>         *)
        (*        sem_arith_policy policy1 packet =         *)
        (*        sem_arith_policy policy2 packet           *)
        (*                                                  *)
        ...
        (* Prove the end-to-end theorem by chaining Theorem 1 (policy1),
           the MTBDD equivalence theorem, and Theorem 1 retranslation (policy2)
