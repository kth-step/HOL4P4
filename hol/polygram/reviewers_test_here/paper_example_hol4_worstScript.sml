...
val _ = new_theory "paper_example_hol4_worst";
...
(* ---------------------------------------------------------------------------
   ...
   P4 interval tables using HOL4 EVAL  MTBDD construction
...
(* Variable grouping G: assigns variables to P4 tables (Section IV-D,
   Algorithm 3). Each group corresponds to one match-action table,
   matching the ip_match, port_match, and ttl_match tables of Figure 5. *)
...
(*  Pipeline invocation                                                    *)
(*                                                                         *)
(*       fwd_proofLib     -- fully inside HOL4 EVAL, no TCB (slow for      *)
(*                           large inputs; see Table II HOL4 columns)      *)
(* ----------------------------------------------------------------------- *)

val final_thm_res_eval =
fwd_proofLib.convert_arith_policy_to_interval_tables (arith_policy_figure1, atoms_map, test_pd_type, variables_grouping, policy_order);  
