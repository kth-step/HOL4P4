...
val _ = new_theory "basic_alt";

val basic_alt_blftymap = ``[("MyParser",[]); ("MyVerifyChecksum",[]);
 ("MyIngress",
...
val basic_alt_ftymap = ``[(funn_ext "ipsec_crypt" "decrypt_null",
...
val basic_alt_pblock_action_names_map = ``[("MyParser",[]); ("MyVerifyChecksum",[]);
 ("MyIngress",
  [("spd",["add_spd_mark"; "drop"]);
   ("forward",["l3_forward"; "l2_forward"; "drop"]);
   ("sad_decrypt",["NoAction"; "esp_decrypt_aes_ctr"; "esp_decrypt_null"]);
   ("sad_encrypt",["esp_encrypt_aes_ctr"; "esp_encrypt_null"; "sadb_acquire"])]);
 ("MyEgress",[]); ("MyComputeChecksum",[]); ("MyDeparser",[])]:((string, ((string, string list) alist)) alist)``;

val basic_alt_actx = ``([arch_block_inp;
 ...
 [("MyParser",pbl_type_parser,
  ...
  ("MyComputeChecksum",pbl_type_control,[("hdr",d_inout); ("meta",d_inout)],
   [("MyComputeChecksum",
     stmt_seq stmt_empty
       (stmt_block
          [(varn_name "no_checksum_ipv4_hdr",
            tau_xtl struct_ty_header
              [("version",tau_bit 4); ("ihl",tau_bit 4);
               ("diffserv",tau_bit 8); ("totalLen",tau_bit 16);
               ("identification",tau_bit 16); ("flags",tau_bit 3);
               ("fragOffset",tau_bit 13); ("ttl",tau_bit 8);
               ("protocol",tau_bit 8); ("srcAddr",tau_bit 32);
               ("dstAddr",tau_bit 32)],NONE)]
          (stmt_seq
             (stmt_ass (lval_varname (varn_name "no_checksum_ipv4_hdr"))
                (e_struct
                   [("1",
                    ...
                    ("5",
                     e_acc (e_acc (e_var (varn_name "hdr")) "ipv4")
                       "identification");
                    ("6",
                     e_acc (e_acc (e_var (varn_name "hdr")) "ipv4") "flags");
                    ("7",
                     e_acc (e_acc (e_var (varn_name "hdr")) "ipv4")
                       "fragOffset");
             ...
             (stmt_ass lval_null
                (e_call (funn_ext "" "update_checksum")
                   [e_call (funn_ext "header" "isValid")
                      [e_acc (e_var (varn_name "hdr")) "ipv4"];
                    e_var (varn_name "no_checksum_ipv4_hdr");
                    e_acc (e_acc (e_var (varn_name "hdr")) "ipv4")
                      "hdrChecksum";
                    e_v
                      (v_bit
...
val b_fty_map' = optionSyntax.dest_some $ rhs $ concl $ EVAL “deparameterise_b_ftymap_entries ^basic_alt_blftymap”

val basic_alt_ty_ctx_tm = “(^fty_map', ^b_fty_map', ^basic_alt_pblock_action_names_map)”
val symb_exec_ty_ctx_def = hd $ Defn.eqns_of $ Defn.mk_defn "ty_ctx" (mk_eq(mk_var("ty_ctx", type_of basic_alt_ty_ctx_tm), basic_alt_ty_ctx_tm))

val basic_alt_pblock_map = #2 $ p4Syntax.dest_actx basic_alt_actx;
val symb_exec_pblock_map_def = hd $ Defn.eqns_of $ Defn.mk_defn "pblock_map" (mk_eq(mk_var("pblock_map", type_of basic_alt_pblock_map), basic_alt_pblock_map))

val basic_alt_wf_sad_encrypt_tbl_tm = “v1model_tbl_is_well_formed ^(lhs $ concl symb_exec_ty_ctx_def) ^(lhs $ concl symb_exec_pblock_map_def) ("sad_encrypt",sad_encrypt_tbl)”

val basic_alt_wf_sad_decrypt_tbl_tm = “v1model_tbl_is_well_formed ^(lhs $ concl symb_exec_ty_ctx_def) ^(lhs $ concl symb_exec_pblock_map_def) ("sad_decrypt",sad_decrypt_tbl)”

val basic_alt_wf_forward_tbl_tm = “v1model_tbl_is_well_formed ^(lhs $ concl symb_exec_ty_ctx_def) ^(lhs $ concl symb_exec_pblock_map_def) ("forward",forward_tbl)”

val basic_alt_wf_spd_tbl_tm = “v1model_tbl_is_well_formed ^(lhs $ concl symb_exec_ty_ctx_def) ^(lhs $ concl symb_exec_pblock_map_def) ("spd",spd_tbl)”
...
val (fty_map, b_fty_map, pblock_action_names_map) = (basic_alt_ftymap, basic_alt_blftymap, basic_alt_pblock_action_names_map);
...
val init_astate = basic_alt_astate_symb
...
val path_cond = ASSUME “^basic_alt_wf_sad_encrypt_tbl_tm /\
                        ^basic_alt_wf_sad_decrypt_tbl_tm /\
                        ^basic_alt_wf_forward_tbl_tm /\
                        ^basic_alt_wf_spd_tbl_tm”
val n_max = 300;
val debug_flag = true;
