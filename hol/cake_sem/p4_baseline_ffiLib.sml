...
(* TODO: Only used when printing debug output
...
val _ = append_prog o process_topdecs $
 ...
 fun packet_to_bool_list packet =
   let
     ...
     (* Pre-compute bit masks for faster lookup *)
     val bit_masks = Vector.fromList [
       Word8.fromInt 128, Word8.fromInt 64, Word8.fromInt 32, Word8.fromInt 16,
       Word8.fromInt 8, Word8.fromInt 4, Word8.fromInt 2, Word8.fromInt 1
     ]

     (* Process multiple bytes at once in chunks *)
     fun process_bytes start acc =
       if start >= len then 
	 List.rev acc
       else
	 let
	   (* Determine chunk size (process up to 16 bytes at once) *)
	   val chunk_end = min (start + 16) len

	   (* Process a single byte and add its bits to the accumulator *)
	   fun process_byte byte acc =
	     let
	       (* Fast bit extraction using pre-computed masks and a single pass *)
	       fun extract_bits i acc =
		 if i >= 8 then acc
		 else
		   let
		     val bit = Word8.andb byte (Vector.sub bit_masks i) <> Word8.fromInt 0
		   in
		     extract_bits (i+1) (bit::acc)
		   end
	     in
	       extract_bits 0 acc
	     end
	   ...
	   fun process_chunk i acc =
	     if i >= chunk_end then acc
	     else process_chunk (i+1) (process_byte (Word8Array.sub packet i) acc)

	   val new_acc = process_chunk start acc
	 in
	   process_bytes chunk_end new_acc
	 end
   in
     process_bytes 0 []
   end

(* Split a list at position n, returns (first n elements, remaining elements) *)
(* TODO: Replaces List.splitAt - why is this not in basis? *)
fun splitAt (lst, n) =
  let
    fun split acc remaining n =
      if n = 0 orelse remaining = [] then
        (List.rev acc, remaining)
      else
        split (List.hd remaining :: acc) (List.tl remaining) (n - 1)
  in
    split [] lst n
  end
 ...
 fun bool_list_to_packet bool_list =
   let
     ...
     (* Pre-compute bit masks for faster setting *)
     val bit_masks = Vector.fromList [
       Word8.fromInt 128, Word8.fromInt 64, Word8.fromInt 32, Word8.fromInt 16,
       Word8.fromInt 8, Word8.fromInt 4, Word8.fromInt 2, Word8.fromInt 1
     ]

     (* Process bits in chunks for better locality *)
     fun process_bits bits offset =
       case bits of
	 [] => ()
       | _ =>
	   let
	     (* Process up to 64 bits (8 bytes) at once *)
	     val chunk_size = min 64 (List.length bits)
	     val (chunk, rest) = splitAt (bits, chunk_size)

	     (* Build bytes for this chunk *)
	     fun build_bytes chunk_bits byte_idx bit_idx =
	       case chunk_bits of
		 [] => ()
	       | b::bs =>
		   let
		     val mask = Vector.sub bit_masks bit_idx
		     val curr_byte = Word8Array.sub packet byte_idx
		     val new_byte = if b then Word8.orb curr_byte mask else curr_byte
		   in
		     Word8Array.update packet byte_idx new_byte;
		     if bit_idx = 7 then
		       build_bytes bs (byte_idx + 1) 0
		     else
		       build_bytes bs byte_idx (bit_idx + 1)
		   end
	   in
	     build_bytes chunk (offset div 8) (offset mod 8);
	     process_bits rest (offset + chunk_size)
	   end
   in
     process_bits bool_list 0;
     packet
...
fun get_baseline_program progname =
 let
  val _ =
    let
