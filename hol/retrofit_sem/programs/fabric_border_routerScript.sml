...
open p4_exec_semProgTheory p4_exec_arch_v1modelProgTheory;
open p4_retrofit_auxLib;

val _ = translation_extends "p4_exec_arch_v1modelProg";
...
val fabric_border_router_actx = “([arch_block_inp;
...
val fabric_border_router_astate = “((0,[],[],0,[],[("parseError",v_bit (fixwidth 32 (n2v 0),32))],
...
val input_bits_tm = p4_cake_auxLib.v1model_get_input fabric_border_router_actx fixed_fields components
...
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 239”

(* 280: Instantiating the direct_counter "acl_pre_ingress_acl_pre_ingress_counter" *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 280”

(* 600: Instantiating "acl_ingress_acl_ingress_meter" *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 599”

(* 700: Applying "acl_ingress.acl_ingress_table" *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 700”

rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 825”

(* 833: Instantiating action selector, start of routing_resolution *)   
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 833”

(* 551: Just before applying ipv4_table *)
#3 $ dest_astate $ dest_some $ rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 551”

rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 552”

rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 1289”

(* TODO: Why is the packet not output in the final stage? *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 1459”



rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 1453”
...
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 139”
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 140”
(* verify_ipv4_checksum block starts at step 143: (~2.5s to EVAL up until this point) *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx'' (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 143”
(* verify_ipv4_checksum finishes at 199 steps *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx'' (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 199”

rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx'' (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 300”
...
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx'' (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 436”
(* 735 steps should output the outgoing packet *)
rhs $ concl $ EVAL “arch_multi_exec ^fabric_border_router_actx (p4_append_input_list [^input_tm] ^fabric_border_router_astate) 735”
...
val actx = fabric_border_router_actx;
val astate = fabric_border_router_astate;
...
Definition arch_multi_exec_debug_def:
 (arch_multi_exec_debug actx (astate:'a astate) 0 =
   (astate, 0))
  /\
 (arch_multi_exec_debug actx astate (SUC fuel) =
  case arch_exec actx astate of
  | SOME astate' =>
   arch_multi_exec_debug actx astate' fuel
  | NONE => (astate, SUC fuel))
End

Question: when is egress_spec set?
It's set to drop at the start of routing_lookup.

Why does ipv4_table try to match with an all-zeroed IPv4 address?

val res = rhs $ concl $ EVAL “arch_multi_exec_debug ^actx (p4_append_input_list [^input] ^astate) 100”;

val res = rhs $ concl $ EVAL “arch_multi_exec_debug ^actx (p4_append_input_list [^input] ^astate) 470”;

(* IPv4 match using

stmt_app "routing_lookup.ipv4_table"
             [e_v (v_bit ([F; F; F; F; F; F; F; F; F; F],10));
              e_v
                (v_bit
                   ([F; F; F; F; T; F; T; F; F; F; F; F; F; F; F; F; F; F; F;
                     F; F; F; F; F; F; F; F; F; F; F; T; F],32))]

        ("routing_lookup.ipv4_table",
         tbl_regular
           [(([s_sing (v_bit ([F; F; F; F; F; F; F; F; F; F],10));
               s_sing
                 (v_bit
                    ([F; F; F; F; T; F; T; F; F; F; F; F; F; F; F; F; F; F;
                      F; F; F; F; F; F; F; F; F; F; F; F; T; F],32))],4),
             "set_nexthop_id",
             [e_v (v_bool T); e_v (v_bool T);
              e_var (varn_name "routing_lookup_local_metadata");
              e_v (v_bit ([F; F; F; F; F; F; F; F; T; F],10))]);
            (([s_sing (v_bit ([F; F; F; F; F; F; F; F; F; F],10));
               s_sing
                 (v_bit
                    ([F; F; F; F; T; F; T; F; F; F; F; F; F; F; F; F; F; F;
                      F; F; F; F; F; F; F; F; F; F; F; F; F; T],32))],4),
             "set_nexthop_id",
             [e_v (v_bool T); e_v (v_bool T);
              e_var (varn_name "routing_lookup_local_metadata");
              e_v (v_bit ([F; F; F; F; F; F; F; F; F; T],10))])]);
...
val res = rhs $ concl $ EVAL “arch_multi_exec_debug ^actx (p4_append_input_list [^input] ^astate) 1075”;
(* egress_port set to drop at least by 1080 steps, when copying over from
 * acl_egress_standard_metadata  *)        
val res = rhs $ concl $ EVAL “arch_multi_exec_debug ^actx (p4_append_input_list [^input] ^astate) 1350”

val res = rhs $ concl $ EVAL “arch_multi_exec_total ^actx (p4_append_input_list [^input] ^astate) 1400”

val res = rhs $ concl $ EVAL “arch_multi_exec_total ^actx (p4_append_input_list [^input] ^astate) 1500”
*)

p4_exec_wrapper_ffiLib.translate_p4 progname actx astate n_max debug_mode inlogic;
